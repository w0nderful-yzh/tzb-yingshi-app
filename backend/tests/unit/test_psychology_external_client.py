...
@pytest.mark.asyncio
async def test_client_reads_latest_completed_projection() -> None:
    requests: list[httpx.Request] = []
    ...
    try:
        result = await source.get_latest_completed_assessment(subject_key="elder-001")
    finally:
        await source.close()

    assert result.status == "completed"
    assert requests[0].url.path == "/api/psychology/assessments/latest-completed"
