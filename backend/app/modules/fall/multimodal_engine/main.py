...
def create_app() -> FastAPI:
    ...
    multimodal_fusion = MultimodalFusionService(
        ...
        minimum_camera_quality=settings.fusion_v2_minimum_camera_quality,
        ...
        associated_evidence_config=AssociatedEvidenceConfig(
            ...
            minimum_track_stability=(settings.fusion_associated_minimum_track_stability),
            weak_vertical_velocity_mps=(settings.fusion_associated_weak_vertical_velocity_mps),
            strong_vertical_velocity_mps=(settings.fusion_associated_strong_vertical_velocity_mps),
        ...
        radar_eligibility_config=RadarEligibilityConfig(
            ...
            history_window_seconds=(settings.fusion_radar_eligibility_history_seconds),
            minimum_track_samples=(settings.fusion_radar_eligibility_minimum_track_samples),
            minimum_point_count=(settings.fusion_radar_eligibility_minimum_point_count),
            reference_point_count=(settings.fusion_radar_eligibility_reference_point_count),
            minimum_track_stability=(settings.fusion_radar_eligibility_minimum_track_stability),
            minimum_radar_quality=(settings.fusion_radar_eligibility_minimum_quality),
            maximum_velocity_jump_mps=(settings.fusion_radar_eligibility_maximum_velocity_jump_mps),
            height_consistency_tolerance_m=(settings.fusion_radar_eligibility_height_tolerance_m),
        ),
    )
    fusion_shadow_sampler = FusionShadowSampler(
        multimodal_fusion.get_latest,
        enabled=(settings.fusion_shadow_log_enabled and settings.fusion_shadow_sampler_enabled),
