...
class ProfileViewModel(private val repo: SafeRepository) : ViewModel() {
    ...
    fun load() {
        viewModelScope.launch {
            ...
            val elderId = repo.getCurrentElderId().getOrElse { return@launch fail(it) }
            // 联系人与设备失败不阻断整页：降级为空列表，由页面展示空态
            val contacts = repo.getContacts(elderId).getOrNull()?.contacts ?: emptyList()
            val devices = repo.getDevices(elderId).getOrNull()?.devices ?: emptyList()
